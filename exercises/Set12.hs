...
instance Functor List where
  fmap f Empty = Empty
  fmap f (LNode n lst) = LNode (f n) (fmap f lst)
...
instance Functor TwoList where
  ...
  fmap f (TwoNode p q lst) = TwoNode (f p) (f q) (fmap f lst)
...
count x = foldr f 0
  where f y acc = acc + fromEnum (x == y)
...
inBoth f g = foldr h [] f
  where h y acc = if (elem y g) then y:acc else acc 
...
instance Foldable List where
  ...
  foldr f init (LNode n lst) = f n $ foldr f init lst
...
instance Foldable TwoList where
  ...
  foldr f init (TwoNode a b lst) = f a (f b $ foldr f init lst)
...
instance Functor Tree where
  fmap f (Leaf) = Leaf
  fmap f (Node a tr1 tr2) = Node (f a) (fmap f tr1) (fmap f tr2)

sumTree :: Monoid m => Tree m -> m
sumTree (Leaf) = mempty
sumTree (Node a tr1 tr2) = (sumTree tr1) <> a <> (sumTree tr2)
