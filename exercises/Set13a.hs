...
split s = 
  if ' ' `elem` s 
  then Just (head $ words s, last $ words s) 
  else Nothing 
...
checkNumber (s1, s2) = 
  if hasDigit s1 || hasDigit s2 
  then Nothing 
  else Just (s1,s2)
  where hasDigit s = length [ch | ch <- s, isDigit ch] > 0 
...
balance accountName = BankOp balHelper
  where balHelper (Bank accounts) = (Map.findWithDefault 0 accountName accounts, Bank accounts) 
...
count x = do
  st <- get
  case lookup x st of Nothing -> put ((x,1):st)
                      Just n  -> put ((x,n+1):filter (\(y,_) -> x /= y) st)
