...
firstHalf :: [Char] -> [Char] 
firstHalf str = reverse (drop (div (length str) 2) (reverse str))
...
joinToLength size lst = filter (\x -> length x == size) [first ++ second | first <- lst, second <- lst]
...
sumRights lst = sum (map (\z -> either (\x -> 0) (\y -> y) z) lst)
...
multiCompose fs val = multiCompose (init fs) ((last fs) val)
...
interpreter commands = case last commands of
  "printX" -> interpreter (init commands) ++ [show $ sum (map xChange commands)]
  "printY" -> interpreter (init commands) ++ [show $ sum (map yChange commands)]
  _ -> interpreter (init commands)

xChange :: String -> Int
xChange cmd = case cmd of 
  "left" -> (-1)
  "right" -> 1
  _ -> 0

yChange :: String -> Int
yChange cmd = case cmd of 
  "up" -> 1
  "down" -> (-1)		   
  _ -> 0
