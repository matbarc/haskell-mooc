...
totalPrice (MkShoppingEntry prod price qty) = price * (fromIntegral qty)
...
buyOneMore (MkShoppingEntry prod price qty) = MkShoppingEntry prod price (qty+1)
...
up pos = MkPosition (x pos) (y pos + 1) 

-- right increases the x value of a position by one
right :: Position -> Position
right pos = MkPosition (x pos + 1) (y pos) 
...
data UpDown = MkInc {val::Int} | MkDec {val::Int}
...
tick :: UpDown -> UpDown
tick ud = case ud of MkInc n -> MkInc (n+1)
		     MkDec n -> MkDec (n-1)

-- toggle changes an increasing counter into a decreasing counter and
-- vice versa
toggle :: UpDown -> UpDown
toggle ud = case ud of MkInc n -> MkDec n
		       MkDec n -> MkInc n
...
toList kvals = case kvals of Empty -> []
			     Pair k v col -> (k,v):toList col
...
fromNat :: Nat -> Int
fromNat n = case n of Zero -> 0
		      PlusOne x -> 1 + fromNat x

toNat :: Int -> Maybe Nat
toNat z 
	| z == 0    = Just (Zero)
	| z > 0     = case n of Nothing -> Nothing
