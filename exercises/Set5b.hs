...
treeSize :: Tree a -> Int
treeSize Empty = 0
treeSize (Node _ tr1 tr2) = 1 + treeSize(tr1) + treeSize(tr2)
...
treeMax Empty = 0
treeMax (Node v tr1 tr2) = max (max v (treeMax tr1)) (treeMax tr2)
...
allValues condition (Node v tr1 tr2) = condition v && 
	all (==True) (map (allValues condition) [tr1, tr2]) 
...
mapTree f (Node v tr1 tr2) = Node (f v) (mapTree f tr1) (mapTree f tr2)
...
cull _ Empty = Empty
cull val (Node v tr1 tr2) = if val == v then Empty else Node v (cull val tr1) (cull val tr2) 
...
walk (step:steps) (Node v l r) = case step of StepL -> case l of Empty  -> Nothing
								 _ -> walk steps l
					      StepR -> case r of Empty  -> Nothing
					      			 _ -> walk steps r
...
set (step:steps) val (Node v l r) = case step of StepL -> Node v (set steps val l) r
						 StepR -> Node v l (set steps val r)
