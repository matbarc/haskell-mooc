...
instance Ord Country where
  compare x y = if x == y then EQ else case x of
    Switzerland -> GT
    Finland -> LT
    Norway -> case y of Switzerland -> LT
			_ -> GT
...
instance Eq Name where
  (==) (Name x) (Name y) = head tup == last tup 
    where tup = map (map Data.Char.toLower) [x,y]
...
instance Eq a => Eq (List a) where
  (==) Empty Empty = True 
  (==) (LNode l llst) (LNode r rlst) = l == r && llst == rlst
  (==) _ _ = False
...
simplify (RationalNumber a b) = 
  if c /= 1 
  then RationalNumber (div a c) (div b c)
