...
add el (Set []) = Set [el]
add el (Set (x:xs)) =
  if member el (Set (x:xs))
  then Set (x:xs)
  else Set (sort (el:x:xs))
...
data State = Start | Egged | Floured | Sugared | MixReady | Mixed | Error | Finished
  deriving (Eq,Show)
...
reverseNonEmpty (x:|[]) = x:|[]
reverseNonEmpty (x:|xs) = head rev :| tail rev where rev = reverse (x:xs)
...
passwordAllowed pw (MinimumLength n) = n <= length pw
passwordAllowed pw (ContainsSome chs) =
  any (==True) $ map (\x -> elem x pw) chs
passwordAllowed pw (DoesNotContain chs) = 
  all (==True) $ map (\x -> notElem x pw) chs
passwordAllowed pw (And req1 req2) = 
  all (==True) $ map (passwordAllowed pw) [req1, req2]
passwordAllowed pw (Or req1 req2) =
  any (==True) $ map (passwordAllowed pw) [req1, req2]
