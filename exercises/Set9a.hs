...
countValid strs = length $ filter f strs
  where f str = (equalChars str 2 4) || (equalChars str 3 5)

equalChars :: String -> Int -> Int -> Bool
equalChars str i j = (str !! i) == (str !! j)
------------------------------------------------------------------------------
...
sumSuccess xs = foldr add (Left "no data") xs

add :: Either String Int -> Either String Int -> Either String Int
add (Right a) (Right b) = Right (a+b)
add (Right a) (Left _) = Right a
add (Left _) (Right a) = Right a
add _ _ = Left "no data"
...
instance Eq Text where
  Text a == Text b = (filter f a) == (filter f b)
    where f = (\x -> not $ isSpace x)
...
compose [] _ = []
compose ((f,s):xs) ys = case query of
  Nothing -> compose xs ys
  Just c  -> (f,c) : (compose xs ys)
  where query = lookup s ys
...
permute ps xs = [xs !! i | (_, i) <- sort (zip ps [0..length ps])]
